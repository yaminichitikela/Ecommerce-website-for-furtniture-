...
if (isset($_GET['DelProductID']) && !empty($_GET['DelProductID'])) {
        ...
        $sql_query = "select count(*) as cntproduct from cart WHERE Product_ID ='{$_GET['DelProductID']}'";
        $result = mysqli_query($con, $sql_query);
        $row = mysqli_fetch_array($result);

        $count = $row['cntproduct'];

        if (isset($_SESSION['EmpName']) && !empty($_SESSION['EmpName']) && $_SESSION['EmpName'] == 'Admin' ) {

            $count =0;
        }

        if ($count <= 0) {
            $sql_delete_product = "DELETE FROM product WHERE Product_ID ='{$_GET['DelProductID']}'";
            ...
            <div class="col-lg-4 col-md-6 col-sm-10 offset-md-0 offset-sm-1">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <a href="product_Management.php?DelProductID=<?php echo $row['Product_ID'] ?>;"><button class="btn w-40 rounded my-2" type="button" <?php echo $deleteVisible ?> onclick="return  confirm('do you want to delete the product?')" id="btnDelete" name="btnDelete">Delete Product</button> </a>
